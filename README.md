...
※ Renderの無料プランが不安定だったため、3/28に有料プランへ移行しました。
...
※ 直近で最も記録が多かった月のExcelシート（2世帯合わせて97件）  
...
- 端数調整ロジックの設計  
  → 電気料金の合計と各メンバーの負担額の整合性を優先し、端数の差分は1名に集約する方式を採用  
  → 将完全均等な見た目ではなく、実際の請求総額と一致することを優先した設計としている  
  → 将来的には、UI上のみ均等に見せる改善も検討
...
※ コメントまみれのapp.pyと問題集：-learning-notes リポジトリより
